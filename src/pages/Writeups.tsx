...
const staticWriteups: Writeup[] = [
  { id: '5', title: "BRK CYS CTF 2026 Write-up", description: "Reverse Engineering and Crypto writeups from BRK CYS CTF 2026...", longDescription: "Step-by-step solutions for four challenges: not_so_simple.c, Dark Matter, Keygen, and Pixel Perfect.", category: "ctf", content_url: "https://medium.com/@amrkhaledv2171516/brk-cys-ctf-2026-bb8e0e8faada", tags: ["CTF", "Reverse Engineering", "XOR", "Anti-Debug", "Crypto", "BRK CYS CTF 2026"], published_date: "2026-04-12", featured: true },
  { id: '4', title: "CyCTF Luxor", description: "Step-by-step write-up of the CyCTF Luxor CTF 2026 Reverse challenges and one mobile challenge, including solutions, and techniques.", category: "ctf", content_url: "https://medium.com/p/2a33d347e0f5?postPublishedType=initial", tags: ["CTF", "Reverse Engineering", "Mobile"], published_date: "2026-03-16", featured: false, created_at: "2026-03-16T00:00:00Z" },
  { id: '3', title: "ZICHMP CTF 2026 - Cyber Champions 2026 Write-up", description: "Step-by-step write-up of the ZICHMP CTF 2026 challenges, including solutions, techniques, and lessons learned.", longDescription: "This detailed write-up covers the ZICHMP CTF 2026 challenges from Cyber Champions 2026, with complete explanations of each solution.", category: "ctf", content_url: "https://medium.com/@amrkhaledv2171516/zichmp-ctf-2026-cyber-champions-2026-abbfe3f13fc6", tags: ["CTF", "Cyber Champions 2026", "Reverse Engineering", "Exploitation", "Network Security"], published_date: "2026-02-08", featured: true, created_at: "2026-02-08T00:00:00Z" },
  { id: '2', title: "Comprehensive Analysis of BadRabbit Ransomware", description: "Full technical dissection of BadRabbit ransomware including kernel-level persistence, network propagation, and advanced encryption mechanisms.", longDescription: "In-depth technical analysis of BadRabbit ransomware covering infection chain, encryption schemes, kernel-level persistence techniques.", category: "malware-analysis", content_url: "https://github.com/Amr-Khaled-Ahmed/Malware-analysis-work-shop/tree/main/task10%20-%20ransomware%20BadRabbit%20full%20analysis", tags: ["Ransomware", "BadRabbit", "Reverse Engineering", "Incident Response", "MITRE ATT&CK"], published_date: "2025-02-27", featured: true, created_at: "2025-02-27T00:00:00Z" },
  { id: '1', title: "PrivExec Privilege Escalation Write-up", description: "Detailed analysis and exploitation of PrivExec for privilege escalation techniques.", longDescription: "A comprehensive write-up covering the exploitation of PrivExec for privilege escalation.", category: "privilege-escalation", content_url: "https://medium.com/@amrkhaledv2171516/privexec-privilege-escalation-write-up-cba868c1a34c", tags: ["Privilege Escalation", "Windows Security", "Exploitation", "Red Teaming"], published_date: "2024-12-15", featured: true, created_at: "2024-12-15T00:00:00Z" },
];
...
export default function Writeups() {
  ...
  const getWriteupCount = (categoryId: CategoryFilter) => {
    ...
    if (categoryId === 'malware') return writeups.filter(w => w.category === 'malware' || w.category === 'malware-analysis').length;
    return writeups.filter(w => w.category === categoryId).length;
  };

  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'malware': case 'malware-analysis': return '🦠';
      case 'ctf': return '🏴';
      case 'bug-bounty': return '💰';
      case 'research': return '🔬';
      case 'network-security': return '🌐';
      case 'digital-forensics': return '🖥️';
  ...
  const getCategoryLabel = (category: string) => {
    switch (category) {
      ...
      case 'malware-analysis': return 'Malware Analysis';
      case 'ctf': return 'CTF';
      case 'bug-bounty': return 'Bug Bounty';
                ...
                <div className="relative p-6 md:p-8">
                  <div className="flex flex-wrap items-center gap-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-4 py-1.5 bg-[#D4AF37]/20 text-[#D4AF37] rounded-full text-sm font-mono font-bold">{getCategoryLabel(writeup.category)}</span>
                    </div>
                    {writeup.featured && <span className="text-2xl text-[#D4AF37]" title="Featured">𓁢</span>}
                    <div className="flex items-center gap-2 text-gray-400 ml-auto">
                      <Calendar size={16} />
                      <span className="text-sm font-mono">
                        {new Date(writeup.published_date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                      </span>
                    </div>
                  </div>
                  <h3 className="text-2xl md:text-3xl font-bold text-white mb-3 group-hover:text-[#D4AF37] transition-colors">{writeup.title}</h3>
                  <p className="text-gray-400 mb-4 text-lg leading-relaxed">{writeup.description}</p>
                  {writeup.longDescription && (
                    <p className="text-gray-500 mb-4 text-sm italic">{writeup.longDescription.substring(0, 150)}...</p>
                  )}
                  {writeup.tags.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2 mb-6">
                      <Tag size={16} className="text-[#2E8B93]" />
                      {writeup.tags.map((tag, i) => (
                        <span key={i} className="px-3 py-1 bg-[#2E8B93]/20 text-[#2E8B93] rounded-full text-sm font-mono hover:bg-[#2E8B93]/30 transition-colors cursor-pointer">#{tag}</span>
                      ))}
                    </div>
                  )}
                  <div className="flex items-center justify-between pt-4 border-t border-[#D4AF37]/20">
                    <a href={writeup.content_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-[#D4AF37] text-[#1B2845] rounded-lg font-bold hover:bg-[#C19A6B] transition-all hover:scale-105 shadow-lg shadow-[#D4AF37]/30">
                      <span>Read Writeup</span><ExternalLink size={18} />
                    </a>
